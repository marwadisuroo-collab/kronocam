...
void main() {
  ...
  img.drawCircle(fg, x: 512, y: 512, radius: 316, color: steel, antialias: true);
  img.fillCircle(fg, x: 512, y: 512, radius: 286, color: dark);
  for (var r = 272; r >= 220; r -= 10) {
    final t = (272 - r) / 52;
    img.fillCircle(fg, x: 512, y: 512, radius: r, color: img.ColorRgba8((60 - 35 * t).round(), (60 - 35 * t).round(), (60 - 35 * t).round(), 255));
  }
  ...
  img.fillCircle(fg, x: 512, y: 512, radius: 153, color: img.ColorRgba8(18, 40, 74, 255));
  for (var i = 0; i < 12; i++) {
    final a = i * math.pi / 6 - math.pi / 2;
    final l = i % 3 == 0 ? 20 : 11;
    img.drawLine(fg, x1: (512 + math.cos(a) * (116 - l)).round(), y1: (512 + math.sin(a) * (116 - l)).round(), x2: (512 + math.cos(a) * 116).round(), y2: (512 + math.sin(a) * 116).round(), color: white, thickness: i % 3 == 0 ? 6 : 3, antialias: true);
  }
  final ten = -math.pi / 3;
  final two = math.pi / 6;
  img.drawLine(fg, x1: 512, y1: 512, x2: (512 + math.cos(ten) * 76).round(), y2: (512 + math.sin(ten) * 76).round(), color: steel, thickness: 10, antialias: true);
  img.drawLine(fg, x1: 512, y1: 512, x2: (512 + math.cos(two) * 88).round(), y2: (512 + math.sin(two) * 88).round(), color: steel, thickness: 8, antialias: true);
  ...
  img.drawCircle(fg, x: 512, y: 512, radius: 257, color: steel, antialias: true);
  img.drawCircle(fg, x: 512, y: 512, radius: 223, color: gun, antialias: true);
  final flattened = img.Image.from(bg);
  img.compositeImage(flattened, fg);
  File('assets/icons/app_icon_foreground.png').writeAsBytesSync(img.encodePng(fg));
  File('assets/icons/app_icon_background.png').writeAsBytesSync(img.encodePng(bg));
